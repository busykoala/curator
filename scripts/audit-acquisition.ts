...
import { cleanArtistName, cleanCatalogText, hasPlausibleCatalogMatch } from "../src/features/sources/musicbrainz";
import { baseCatalogTitle } from "../src/features/sources/musicbrainz";
...
assert.equal(cleanCatalogText("Photograph (Single)"), "Photograph");
assert.equal(cleanCatalogText("Pork and Beans (Promo CDS)"), "Pork and Beans");
assert.equal(cleanCatalogText("Joe Cocker's Greatest Hits (A&M 3257)"), "Joe Cocker's Greatest Hits");
assert.equal(baseCatalogTitle("Photograph (Single)"), "Photograph");
const elephunk={id:"elephunk",title:"Elephunk","primary-type":"Album","artist-credit":[{artist:{id:"peas",name:"Black Eyed Peas"}}],score:100};
assert.equal(hasPlausibleCatalogMatch("Elephunk",[{...elephunk,id:"compilation",title:"Behind the Bridge to Elephunk"}]),false);
assert.equal(hasPlausibleCatalogMatch("Elephunk",[elephunk]),true);
assert.equal(identityIsWritable(resolveIdentity("TheBlack Eyed Peas","Elephunk",[elephunk],[])),true);
...
const colorIdentity=resolveIdentity("Weezer","Weezer (Red Album)",[{id:"red",title:"Weezer",disambiguation:"Red Album","primary-type":"Album","artist-credit":[{artist:{id:"artist",name:"Weezer"}}],score:100}],[]);
assert.equal(colorIdentity.album,"Weezer (Red Album)");
assert.equal(identityIsWritable(colorIdentity),true);
const redundantArtistIdentity=resolveIdentity("Al Green","Al Green Explores Your Mind",[{id:"mind",title:"Explores Your Mind","primary-type":"Album","artist-credit":[{artist:{id:"al",name:"Al Green"}}],score:100}],[]);
assert.equal(redundantArtistIdentity.album,"Explores Your Mind");
assert.equal(identityIsWritable(redundantArtistIdentity),true);
