...
export async function GET(request:Request){
  if(!await authenticated())return new Response("Unauthorized",{status:401});reconcileIssues();const url=new URL(request.url),filter=url.searchParams.get("filter")??"all",limit=Math.min(5000,Math.max(1,Number(url.searchParams.get("limit")??5000))),selected=codes[filter],base=`SELECT min(i.id) id,i.album_key,i.code,max(i.severity) severity,max(i.message) message,max(i.updated_at) updated_at,(SELECT min(artist_name) FROM files WHERE album_key=i.album_key) artist,(SELECT min(album_name) FROM files WHERE album_key=i.album_key) album,coalesce(nullif(count(DISTINCT i.file_id),0),(SELECT count(*) FROM files WHERE album_key=i.album_key)) tracks FROM issues i WHERE i.status='open'`;
