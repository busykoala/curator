...
type ExistingFile = { id: number; path: string; inode: number; size: number; mtime_ms: number; status:string };
...
async function processFile(path: string, workers: TagWorkerPool, existing?: ExistingFile): Promise<"new" | "changed" | "unchanged" | "failed"> {
  try {
    ...
    const result = db().prepare(`INSERT INTO files(path,album_key,artist_name,album_name,format,inode,link_count,size,mtime_ms,tags_json,properties_json,artwork_json,status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?, 'analyzed') ON CONFLICT(path) DO UPDATE SET album_key=excluded.album_key,artist_name=excluded.artist_name,album_name=excluded.album_name,format=excluded.format,inode=excluded.inode,link_count=excluded.link_count,size=excluded.size,mtime_ms=excluded.mtime_ms,tags_json=excluded.tags_json,properties_json=excluded.properties_json,artwork_json=excluded.artwork_json,status='analyzed',last_seen_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP RETURNING id`).get(path, key, artist, album, extname(path).slice(1), Number(metadata.ino), metadata.nlink, metadata.size, metadata.mtimeMs, JSON.stringify(tagRecord), JSON.stringify(properties), JSON.stringify(pictures)) as { id: number };
    analyzeFile(result.id, key, tagRecord, pictures); return existing ? "changed" : "new";
