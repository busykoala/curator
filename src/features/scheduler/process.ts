...
function transientFailure(error:unknown):boolean{return /(?:\b429\b|\b5\d\d\b|temporar(?:y|ily)|timed?\s*out|timeout|econn|connection error|fetch failed|network|no router for requested model)/i.test(String(error))}
...
export async function processAlbums(limit=12,report:(progress:Progress)=>void=()=>{}):Promise<{albums:number;written:number}>{
  ...
  for(let albumIndex=0;albumIndex<keys.length;albumIndex++){
    ...
    try{
      ...
      await ensureArtistImage(key,writableIdentity?identity.artistId:undefined,artistDir,identity.artist);
      ...
      const spokenWord=Boolean(db().prepare("SELECT 1 FROM track_profiles p,json_each(p.profile_json,'$.genre') genre WHERE p.album_key=? AND genre.value='spoken_word_comedy' LIMIT 1").get(key));
      if(!writableIdentity&&!manuallyConfirmed&&!spokenWord)db().prepare("INSERT INTO issues(album_key,code,severity,message) VALUES (?,'identity_unresolved','warning',?)").run(key,`Best identity confidence ${identity.confidence.toFixed(2)}, margin ${identity.margin.toFixed(2)}; no factual IDs written`);
