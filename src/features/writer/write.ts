...
function acknowledgeCuratorWrite(fileId:number):void{db().prepare("UPDATE track_profiles SET source_updated_at=(SELECT updated_at FROM files WHERE id=?) WHERE file_id=?").run(fileId,fileId)}
...
export async function writeMetadata(file:FileRow,desired:DesiredTrackMetadata):Promise<"written"|"unchanged">{
  ...
  const current=await stat(file.path);if(current.size!==file.size||current.mtimeMs!==file.mtime_ms)throw new Error("File changed after scan; rescan required");
  // TagLib determines the container from the filename. Keep the original
  // extension on the transactional copy or MP3 writes fail before parsing.
  const extension=extname(file.path),stem=extension?file.path.slice(0,-extension.length):file.path,temp=`${stem}.curator-${process.pid}-${Date.now()}${extension}`;
